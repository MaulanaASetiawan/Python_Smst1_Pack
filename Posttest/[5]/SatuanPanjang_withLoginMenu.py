...
# Tempat Menampung Data Registrasi
login = {
    'user': '',
    'pass': ''}

percobaan = 3 # Batas Percobaan Login
...
def awal():
    ...
    if pilih_menu == '1':
        ...
        input('Tekan ENTER untuk kembali...')
        awal()
# Menu Registrasi Account
def reg():
        login['user'] = str(input('Masukkan Username : '))
        login['pass'] = str(input('Masukkan Password : '))
        input('Tekan ENTER untuk ke menu Login')
        awal()
# Menu Login
def Login():
    for i in range(1,percobaan+1):
        ...
        if a == login['user'] and b == login['pass']:
            menu()
            
        else:
            
            print('\nUsername atau Password Salah')
            print(f'Ini adalah percobaan ke-{i}')
            
    else:
        print('='*30)
        print(f'Anda sudah salah sebanyak {percobaan}x')
        print('Anda akan keluar dari program dalam 3 detik !')
        time.sleep(3)
        exit()
